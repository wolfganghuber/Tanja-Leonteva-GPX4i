...
For this, we pivot `xn` to long format, resulting in `xnl`.

```{r}
#| label: pivot
xnl = tidyr::pivot_longer(xn, cols = JEKO_DMSO:MAVER_T0)
xnl    
dplyr::filter(xnl, Gene == "ABCG1") |>
  mutate(sgRNA = factor(sgRNA)) |>
  ggplot(aes(y = value, col = sgRNA)) +
    geom_point(x = name) + geom_line(x = as.integer(name))
```
